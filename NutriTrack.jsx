...
export default function NutriTrack() {
  ...
  const [swUpdateReady, setSwUpdateReady] = useState(false); // true when a new SW is waiting
  const swRegRef = useRef(null); // holds the SW registration for skipWaiting
  ...
  useEffect(() => {
    ...
    navigator.serviceWorker.getRegistration().then(reg => {
      ...
      // Already waiting (e.g. user came back to a tab with a pending update)
    ...
    // Listen for controller change after skipWaiting so we can reload
      ...
      <div style={S.app}>
        {swUpdateReady && (
          <div style={{background:\"#1e3a5f\",borderBottom:\"1px solid #3b82f6\",padding:\"10px 14px\",display:\"flex\",justifyContent:\"space-between\",alignItems:\"center\",gap:8,flexShrink:0,zIndex:200}}>
            <div style={{fontSize:12,color:\"#93c5fd\",lineHeight:1.5,flex:1}}>🔄 New version available.</div>
            <button style={{background:\"#3b82f6\",border:\"none\",borderRadius:8,color:\"#fff\",fontSize:12,fontWeight:700,cursor:\"pointer\",padding:\"6px 12px\",flexShrink:0}} onClick={() => { if (swRegRef.current && swRegRef.current.waiting) swRegRef.current.waiting.postMessage({type:\"SKIP_WAITING\"}); }}>Reload</button>
          </div>
        )}
        {!isOnline && (
