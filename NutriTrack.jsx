...
export default function NutriTrack() {
  ...
  const moveIngInRecipe = (idx, dir) => setRecipeInProgress(prev => { const arr=[...prev.ingredients]; const j=idx+dir; if (j<0||j>=arr.length) return prev; [arr[idx],arr[j]]=[arr[j],arr[idx]]; return { ...prev, ingredients:arr }; });
  const logRecipe = () => {
      ...
      <div style={S.app}>
        ...
        <div style={S.section}>
          ...
          <div style={S.card}>
            ...
            {recipeInProgress.ingredients.length===0?(<div style={{textAlign:"center",padding:"20px 0",color:"#475569",fontSize:13}}>No ingredients yet — tap + Add</div>):recipeInProgress.ingredients.map((ing,i)=>{const food=allFoods.find(f=>f.id===ing.foodId),ingCal=food?fmtE(food.cal*ing.amount_g/100):0;return(<div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 0",borderBottom:i<recipeInProgress.ingredients.length-1?"1px solid #1e293b":"none"}}><div style={{display:"flex",alignItems:"center",gap:4,flex:1,minWidth:0}}><div style={{display:"flex",flexDirection:"column"}}><button style={{background:"none",border:"none",color:i>0?"#94a3b8":"#334155",fontSize:12,lineHeight:1,cursor:i>0?"pointer":"default",padding:"1px 2px"}} disabled={i===0} onClick={()=>moveIngInRecipe(i,-1)}>▲</button><button style={{background:"none",border:"none",color:i<recipeInProgress.ingredients.length-1?"#94a3b8":"#334155",fontSize:12,lineHeight:1,cursor:i<recipeInProgress.ingredients.length-1?"pointer":"default",padding:"1px 2px"}} disabled={i===recipeInProgress.ingredients.length-1} onClick={()=>moveIngInRecipe(i,1)}>▼</button></div><div style={{minWidth:0}}><div style={{fontSize:13,color:"#e2e8f0",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{ing.foodName}</div><div style={{fontSize:11,color:"#64748b"}}>{ingCal} {energyLabel}</div></div></div><div style={{display:"flex",alignItems:"center",gap:6,flexShrink:0}}><input style={{...S.input,width:64,textAlign:"center",padding:"6px 8px",marginBottom:0}} type="number" inputMode="decimal" value={ing.amount_g} onChange={e=>updateIngAmountInRecipe(i,e.target.value)}/><span style={{fontSize:12,color:"#64748b"}}>g</span><button style={{background:"none",border:"none",color:"#ef4444",fontSize:18,cursor:"pointer",padding:"4px 8px"}} onClick={()=>removeIngFromRecipe(i)}>×</button></div></div>);})}
          </div>
