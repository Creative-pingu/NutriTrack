              ...
              <div style={S.card}>
                ...
                {selectedFood.servings && selectedFood.servings.length > 0 && (
                  <div style={{marginTop:8}}>
                    <div style={{fontSize:11,color:"#64748b",marginBottom:4}}>Serving presets</div>
                    <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                      {selectedFood.servings.map(s => (
                        <button key={s.label} style={S.pill(amount===String(s.grams))} onClick={() => setAmount(String(s.grams))}>{s.label}</button>
                      ))}
                    </div>
                  </div>
                )}
            ...
            <div style={S.card}>
              ...
              {recipeIngSelected.servings && recipeIngSelected.servings.length > 0 && (
                <div style={{marginTop:8}}>
                  <div style={{fontSize:11,color:"#64748b",marginBottom:4}}>Serving presets</div>
                  <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                    {recipeIngSelected.servings.map(s => (
                      <button key={s.label} style={S.pill(recipeIngAmount===String(s.grams))} onClick={() => setRecipeIngAmount(String(s.grams))}>{s.label}</button>
                    ))}
                  </div>
                </div>
              )}
