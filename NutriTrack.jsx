...
export default function NutriTrack() {
  ...
  const swLog = msg => setSwDebug(prev => [...prev, `${new Date().toISOString().slice(11,19)} ${msg}`]);
  ...
  useEffect(() => {
    if (!("serviceWorker" in navigator)) { swLog("SW: not supported"); return; }
    swLog("SW: starting");
    swLog("SW: _swReady=" + !!window._swReady);
    (window._swReady || navigator.serviceWorker.getRegistration()).then(reg => {
      if (!reg) { swLog("SW: no registration"); return; }
      swLog("SW: reg scope=" + reg.scope);
      swLog("SW: controller=" + !!navigator.serviceWorker.controller);
      swLog("SW: waiting=" + !!reg.waiting + " active=" + !!reg.active);
      swRegRef.current = reg;
      if (reg.waiting) { swLog("SW: waiting → banner"); setSwUpdateReady(true); }
      reg.addEventListener("updatefound", () => {
        const sw = reg.installing;
        swLog("SW: updatefound installing=" + !!sw);
        if (!sw) return;
        sw.addEventListener("statechange", () => {
          swLog("SW: state=" + sw.state + " controller=" + !!navigator.serviceWorker.controller);
          if (sw.state === "installed" && navigator.serviceWorker.controller) {
            swRegRef.current = reg;
            swLog("SW: ready → banner");
    ...
    navigator.serviceWorker.addEventListener("controllerchange", () => {
      swLog("SW: controllerchange → reload");
  ...
  const swDebugOverlay = swDebug.length > 0 && (
    <div style={{position:"fixed",bottom:90,left:8,right:8,background:"rgba(0,0,0,0.92)",border:"1px solid #334155",borderRadius:8,padding:"8px 10px",zIndex:9999,maxHeight:180,overflowY:"auto"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
        <span style={{fontSize:10,fontWeight:700,color:"#f59e0b"}}>SW DEBUG</span>
        <button onClick={() => setSwDebug([])} style={{fontSize:10,color:"#64748b",background:"none",border:"none",cursor:"pointer"}}>clear</button>
      </div>
      {swDebug.map((line,i) => <div key={i} style={{fontSize:10,color:"#94a3b8",fontFamily:"monospace",lineHeight:1.6}}>{line}</div>)}
    </div>
  );
  const globalBanners = (<>
    {swDebugOverlay}
