...
export default function NutriTrack() {
  ...
  useEffect(() => {
    ...
    const onOnline  = () => { setIsOnline(true);  probe(); };
  ...
  const logRecipe = () => {
    ...
    const di = selectedRecipe.ingredients.map(ing => {
      ...
      return { foodId:ing.foodId, foodName:ing.foodName, amount_g:Math.round(ing.amount_g*frac*10)/10, ...(food ? { snapshot:buildFoodSnapshot(food) } : {}) };
    });
    // Route to ingredient review before committing (W1)
    ...
    return (
      <div style={S.app}>
        ...
        <div style={S.macroGrid}>
          {(() => {
            // W2 — compute null arc fractions for all tracked nutrients
            // After W0: null = genuine data gap; 0.0 = confirmed absent. Only null triggers indicator.
            const ALL_KEYS = [...MACROS, ...MICROS];
            const nullFoodsByKey = {};
            const nullArcByKey  = {};
            ALL_KEYS.forEach(k => {
              const goal = effectiveGoals[k] || 1;
              // Median of non-null, non-zero values across the full food DB
              ...
              if (nullFoods.length) {
                nullFoodsByKey[k] = [...new Set(nullFoods)];
                nullArcByKey[k]   = Math.min(estTotal/goal, 1);
              }
            });

            return MACROS.map(k => {
              const arc = nullArcByKey[k] || 0;
              const hasNull = !!nullFoodsByKey[k];
              return (
        ...
        {nullPanelKey && MACROS.includes(nullPanelKey) && (() => {
          const meta = NUTRIENT_META[nullPanelKey];
          const foods = (nullPanelKey && ({})[nullPanelKey]) || []; // resolved below via closure
          // Re-derive foods list from state — nullPanelKey is the key
          const nullFoods = (() => {
            const k = nullPanelKey, all = allFoodsForRender;
            const found = [];
            dayLog.forEach(e => {
              if (e.type==="exercise"||e.type==="supplement") return;
              if (e.type==="recipe") { (e.derivedIngredients||[]).forEach(ing => { const v=ing.snapshot?ing.snapshot[k]:(all.find(f=>f.id===ing.foodId)?.[k]); if(v===null||v===undefined) found.push(ing.foodName); }); return; }
              const v=e.snapshot?e.snapshot[k]:(all.find(f=>f.id===e.foodId)?.[k]);
              if(v===null||v===undefined) found.push(e.foodName||"Unknown");
            });
          ...
          return (
            <div style={{margin:"0 16px 4px",background:"#0f1a2e",border:"1px solid #1d4ed8",borderRadius:10,padding:"10px 14px",fontSize:12,color:"#93c5fd",lineHeight:1.5}}>
              ...
              <div style={{color:"#7dd3fc",marginBottom:6}}>{nullFoods.length} food{nullFoods.length===1?"":"s"} had no data: {nullFoods.join(", ")}.</div>
        ...
        <div style={S.section}>
          <div style={S.card}>
            ...
            {(() => {
              // Re-use the nullFoodsByKey/nullArcByKey computed once above — but we're inside a separate
              // render block so recompute for micros only (cheap — same logic, MICROS subset)
              const nullFoodsByMicro = {};
              const nullArcByMicro  = {};
              MICROS.forEach(k => {
                const goal = effectiveGoals[k] || 1;
                const nonNullVals = allFoodsForRender.map(f => f[k]).filter(v => v != null && v > 0);
                nonNullVals.sort((a,b)=>a-b);
                ...
                dayLog.forEach(e => {
                  ...
                  if (e.type==="recipe") {
                    (e.derivedIngredients||[]).forEach(ing => {
                      const val = ing.snapshot ? ing.snapshot[k] : (allFoodsForRender.find(f=>f.id===ing.foodId)?.[k]);
                      if (val===null||val===undefined) { const est=median!=null?(ing.amount_g/100)*median:(totalWeight>0?(ing.amount_g/totalWeight)*goal*0.1:0); if(est/goal>=0.05){nullFoods.push(ing.foodName);estTotal+=est;} }
                    }); return;
                  }
                  const val = e.snapshot ? e.snapshot[k] : (allFoodsForRender.find(f=>f.id===e.foodId)?.[k]);
                  if (val===null||val===undefined) { const est=median!=null?((e.amount||0)/100)*median:(totalWeight>0?((e.amount||0)/totalWeight)*goal*0.1:0); if(est/goal>=0.05){nullFoods.push(e.foodName||"Unknown");estTotal+=est;} }
                });
                if (nullFoods.length) { nullFoodsByMicro[k]=[...new Set(nullFoods)]; nullArcByMicro[k]=Math.min(estTotal/goal,1); }
              });
              return MICROS.map(k => {
                const p=pct(k), meta=NUTRIENT_META[k], hasNull=!!nullFoodsByMicro[k];
                return (
                  <div key={k}>
                    <div style={S.microRow} onClick={() => {
                      ...
                      <div style={S.microBar}>
                        <div style={{position:"absolute",left:0,top:0,height:"100%",width:`${Math.min(p,100)}%`,background:meta.color,borderRadius:2,transition:"width 0.5s ease"}}/>
                        {hasNull && displayMode==="advanced" && (() => {
                          const arc = nullArcByMicro[k];
                    ...
                    {hasNull && nullPanelKey===k && (
                      <div style={{background:"#0f1a2e",border:"1px solid #1d4ed8",borderRadius:8,padding:"8px 10px",margin:"2px 0 6px",fontSize:12,color:"#93c5fd",lineHeight:1.5}}>
                        <div style={{fontWeight:700,marginBottom:4}}>{meta.label} may be higher than shown.</div>
                        <div style={{color:"#7dd3fc",marginBottom:4}}>{nullFoodsByMicro[k].length} food{nullFoodsByMicro[k].length===1?"":"s"} had no data: {nullFoodsByMicro[k].join(", ")}.</div>
  ...
  if (view === "recipeLogReview" && selectedRecipe) {
    ...
    return (
      <div style={S.app}>
        ...
        <div style={S.section}>
          <div style={{fontSize:14,fontWeight:600,color:"#a78bfa",marginBottom:4}}>📖 {selectedRecipe.name}</div>
          <div style={{fontSize:12,color:"#64748b",marginBottom:12}}>Remove or adjust quantities. Changes apply to this log entry only — the saved recipe is unchanged.</div>
          <div style={S.card}>
              ...
              return (
                <div key={idx} style={{display:"flex",alignItems:"center",gap:8,padding:"10px 0",borderBottom:idx<recipeLogReviewIngredients.length-1?"1px solid #1e293b":"none"}}>
                  <div style={{flex:1,minWidth:0}}>
                    ...
                    <div style={{fontSize:11,color:"#64748b"}}>{ingCal} kcal</div>
                  </div>
                  <input
        ...
        <div style={S.section}>
          ...
          <div style={S.card}>
            ...
            <div style={{background:"#0a0f1a",borderRadius:10,padding:14,marginBottom:16}}>
              <div style={{fontSize:11,fontWeight:700,color:"#475569",marginBottom:10,textTransform:"uppercase"}}>Estimated Burn</div>
              <div style={{display:"flex",alignItems:"center",gap:12}}>
                <div style={{fontSize:32,fontWeight:700,color:"#4ade80"}}>{previewBurn}</div>
              ...
              <div style={{marginTop:12}}>
                ...
                <input style={S.input} type="number" inputMode="numeric" placeholder={"Auto: "+autoBurn+" kcal"} value={editLogBurn} onChange={e => setEditLogBurn(e.target.value)}/>
              </div>
            </div>
