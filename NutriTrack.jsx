...
export default function NutriTrack() {
  ...
  useEffect(() => {
    ...
    // ── Diagnostic logging (active when ?swdebug=1) ───────────────────────
    const swDebug = new URLSearchParams(window.location.search).get("swdebug") === "1";
    const swLog = (msg) => {
      if (!swDebug) return;
      ...
      try {
        const prev = JSON.parse(localStorage.getItem("nt-swdebug-log") || "[]");
        prev.push(entry);
        localStorage.setItem("nt-swdebug-log", JSON.stringify(prev.slice(-60)));
      } catch(e) {}
    };

    // Receive SW_LOG messages from the service worker
    navigator.serviceWorker.addEventListener("message", (event) => {
      if (event.data && event.data.type === "SW_LOG") {
        try {
          const prev = JSON.parse(localStorage.getItem("nt-swdebug-log") || "[]");
          prev.push(event.data.entry);
          localStorage.setItem("nt-swdebug-log", JSON.stringify(prev.slice(-60)));
        } catch(e) {}
      }
    });

    swLog("JSX useEffect: running");
    swLog(`JSX useEffect: window._swReady = ${typeof window._swReady}`);

    (window._swReady || navigator.serviceWorker.getRegistration()).then(reg => {
      if (!reg) { swLog("JSX useEffect: getRegistration() returned undefined"); return; }
      swLog(`JSX useEffect: got registration — installing=${reg.installing?.state ?? "null"} waiting=${reg.waiting?.state ?? "null"} active=${reg.active?.state ?? "null"}`);
      swRegRef.current = reg;
      if (reg.waiting) {
        swLog("JSX useEffect: reg.waiting exists at attach time — calling setSwUpdateReady");
        setSwUpdateReady(true);
      }
      reg.addEventListener("updatefound", () => {
        const sw = reg.installing;
        swLog(`JSX updatefound: new SW installing — state=${sw?.state ?? "null"}`);
        if (!sw) return;
        sw.addEventListener("statechange", () => {
          swLog(`JSX statechange: SW state=${sw.state}, controller=${!!navigator.serviceWorker.controller}`);
          if (sw.state === "installed" && navigator.serviceWorker.controller) {
            swLog("JSX statechange: conditions met — calling setSwUpdateReady");
            swRegRef.current = reg;
            setSwUpdateReady(true);
          }
        });
    ...
    navigator.serviceWorker.addEventListener("controllerchange", () => {
      swLog("JSX controllerchange: reloading");
  ...
  // ── SW DEBUG OVERLAY (visible only when ?swdebug=1) ────────────────
  const swDebugOverlay = (() => {
    if (new URLSearchParams(window.location.search).get("swdebug") !== "1") return null;
    let logs = [];
    try { logs = JSON.parse(localStorage.getItem("nt-swdebug-log") || "[]"); } catch(e) {}
    return ReactDOM.createPortal(
      <div style={{position:"fixed",bottom:0,left:0,right:0,maxHeight:"40vh",overflowY:"auto",background:"rgba(0,0,0,0.92)",zIndex:9999,padding:"8px 10px",fontFamily:"monospace",fontSize:11,color:"#a3e635",borderTop:"2px solid #a3e635"}}>
        <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}>
          <strong style={{color:"#fff"}}>SW Debug Log</strong>
          <button style={{background:"#dc2626",border:"none",color:"#fff",fontSize:11,padding:"2px 8px",borderRadius:4,cursor:"pointer"}}
            onClick={() => { localStorage.removeItem("nt-swdebug-log"); window.location.reload(); }}>
            Clear
          </button>
        </div>
        {logs.length === 0
          ? <div style={{color:"#64748b"}}>No log entries yet. Reload to capture events.</div>
          : logs.map((entry, i) => <div key={i} style={{borderBottom:"1px solid #1e293b",padding:"2px 0"}}>{entry}</div>)
        }
      </div>,
      document.body
    );
      ...
      <div style={S.app}>
        ...
        <div style={S.section}>
          ...
          <div style={S.card}>
            ...
            {recipeInProgress.ingredients.length===0?(<div style={{textAlign:"center",padding:"20px 0",color:"#475569",fontSize:13}}>No ingredients yet — tap + Add</div>):recipeInProgress.ingredients.map((ing,i)=>{const food=allFoods.find(f=>f.id===ing.foodId),ingCal=food?fmtE(food.cal*ing.amount_g/100):0;return(<div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 0",borderBottom:i<recipeInProgress.ingredients.length-1?"1px solid #1e293b":"none"}}><div style={{flex:1}}><div style={{fontSize:13,color:"#e2e8f0"}}>{ing.foodName}</div><div style={{fontSize:11,color:"#64748b"}}>{ing.amount_g}g · {ingCal} {energyLabel}</div></div><button style={{background:"none",border:"none",color:"#ef4444",fontSize:18,cursor:"pointer",padding:"4px 8px"}} onClick={()=>removeIngFromRecipe(i)}>×</button></div>);})}
          </div>
