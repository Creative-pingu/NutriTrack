...
export default function NutriTrack() {
  ...
  const logRecipe = () => {
    ...
    if (frac <= 0) return; // guard: canLog on recipeLog screen should prevent this
    const di = selectedRecipe.ingredients.map(ing => {
      const food = allFoods.find(f => f.id === ing.foodId);
      // Round to 1dp but floor at 0.1g so no ingredient silently becomes 0
      const rawAmt = ing.amount_g * frac;
      const amt = rawAmt > 0 ? Math.max(Math.round(rawAmt * 10) / 10, 0.1) : 0;
