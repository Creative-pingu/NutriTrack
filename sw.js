...
const CACHE_VERSION = "nutritrack-v40";
...
// ── Diagnostic logging — broadcasts to all clients when they have ?swdebug=1 ──
async function swLog(msg) {
  const ts = new Date().toISOString().slice(11, 23);
  const entry = `[${ts}] SW: ${msg}`;
  console.log("[SWDebug]", entry);
  const clients = await self.clients.matchAll({ includeUncontrolled: true, type: "window" });
  for (const client of clients) {
    if (client.url.includes("swdebug=1")) {
      client.postMessage({ type: "SW_LOG", entry });
    }
  }
}
...
self.addEventListener("install", event => {
  swLog(`install fired — CACHE_VERSION=${CACHE_VERSION}`);
  event.waitUntil(
...
self.addEventListener("activate", event => {
  swLog("activate fired");
  event.waitUntil(
    caches.keys().then(keys =>
      Promise.all(
        keys
          .filter(k => k !== CACHE_VERSION)
          .map(k => { swLog(`deleting old cache: ${k}`); return caches.delete(k); })
      )
    ).then(() => { swLog("clients.claim() called"); return self.clients.claim(); })
  );
});
...
self.addEventListener("message", event => {
  if (event.data && event.data.type === "SKIP_WAITING") {
    swLog("skipWaiting() called via message");
    self.skipWaiting();
  }
});
